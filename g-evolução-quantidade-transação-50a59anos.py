...
df_50a59 = df[df["PAG_IDADE"].str.contains("entre 50 e 59 anos")]


df_mensal = df_50a59.groupby("MES")["QUANTIDADE"].sum().reset_index()
df_mensal = df_mensal.sort_values("MES")
df_mensal["MES"] = pd.to_datetime(df_mensal["MES"], format="%Y%m")

plt.figure(figsize=(12, 6))
plt.plot(df_mensal["MES"], df_mensal["QUANTIDADE"], marker="o", color="darkred", label="entre 50 e 59 anos")

plt.title("Evolução mensal das transações de pessoas entre 50 e 59 anos", fontsize=14)
plt.xlabel("Mês")
